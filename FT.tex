...
Before proving that $\WFT$ is realized, a continuity lemma is needed.
...
Using this lemma and the $\lambda$-terms from the previous subsection, we can now realize $\WFT$.

\begin{theorem}[Realization of Fan Theorem]\label{thm:FT}
  Using our previous definitions:
  \[\lambda b. n_{C_b} \realU \WFT\]
\end{theorem}

\begin{proof}
  Let's prove that $\lambda b. n_{C_b} \realU\WFT$. Let $\enviro\in\Oracle$ be a reduction context of length $m$ and
  ...
  but we already know that $n_{C_b}$ either reduces to an integer or diverges. If it reduces to an integer, then this integer is a uniform bound for $C$, but as $C \subseteq B$, it follows that $n_{C_b}$ is a uniform bound for $B$.

  Now, suppose $n_{C_b}$ diverges. This means that for any $n \in \bN$, there exists some word $w_n \in \bB^\star$ such that $w_n \notin C$. By applying Weak König's Lemma (in our meta-theory), we find an infinite path
  \[\alpha : \bN \to \bB\]
  ...
  \[n_{C_b}\realP{\enviro}{} \exists n^{\{\Nat\}}, \forall \alpha^{\{\Nat \to \Bool\}}, \restr\alpha n \in B\]
  which means that $\lambda b.n_{C_b}\realU \WFT$.
\end{proof}
