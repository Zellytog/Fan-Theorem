...
We now define the realizability relation $\real$. For this, we must first give a Heyting pre-algebra interpreting $\Prop$. This will be done, given a reduction context $\enviro$, by using $\SATE$. Then, we define the set of codes of any element $s \in \bS$, for a given sort $\bS$. This set will depend on $\enviro$, as oracles add new code.
...
\begin{definition}[Code of an element]
  ...
  \item if $S = \Prop$, then for all $A \in \SAT$,
    \[\codE A \defeq A\]
  \end{itemize}

  Finally, we define for all element $s \in \bS$:
  \[\cod{s} \defeq \bigcap_{\enviro \in \Oracle} \codE{s}\]
\end{definition}

The fact that $\codE s \in\SAT$ is a straightforward induction on the set of sorts.
...
\begin{notation}
  ...
  \begin{align*}
    ...
    \realUFE{\varphi} &\defeq \{ t \in \Lambda \mid t \realUP{\termenv}\varphi \} \\
  \end{align*}
\end{notation}
