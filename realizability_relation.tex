...
\begin{notation}
  ...
  This means that, if $t\realP{\enviro}{\termenv} \exists\varx^{\{S\}}, \varphi(\varx)$, then there exists some $s \in \bS$ such that $\pi_1\;t$ is a code of $s$ (for the reduction context $\enviro$) and $\pi_2\;t \realP{\enviro}{\termenv[\vary\mapsto s]} \varphi(\vary)$ with $\vary$ a fresh variable.
\end{notation}
