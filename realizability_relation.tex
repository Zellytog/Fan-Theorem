...
We now have a semantic interpretation of our initial HOL syntax, where propositions are sets of terms. The realizability relation then defines, for a syntactic proposition $\vdash \varphi : \Prop$, an object $\sem{\varphi} \in \sem{\Prop}$. If we unfold the definition of $\sem{\varphi}$, we find that it is defined as an element of $\SAT$ the way we expect in the usual realizability construction, except for our altered definition of $\impliesSAT$.%In a way, the relation is simply $t \in \cod{\rho(\varphi)}$ for $\Gamma\vdash \varphi : \Prop$ and $\rho\models \Gamma$, but we prefer to give an explicit definition of this relation.

\begin{definition}[Realizability relation]\label{def:real}
  Let $\enviro \in \Oracle$, $\Gamma \in \Hctx$, $\termenv \models \Gamma$ and $\Gamma\vdash \varphi : \Prop$. We define by the realizability relation by
  $t\realPP \varphi \defeq t \in \codE{\rho(\varphi)}$.
  %\begin{itemize}
  ...
  We also define the universal realizability relation by $t\realUP{\termenv} \varphi \defeq t\in\cod{\rho(\varphi)}$. Remark that it is equivalent to saying that for any $\enviro\in\Oracle, t\realPP\varphi$.
...
%  \[
%    \realFP{\varphi}{\termenv}{} \defeq \{ t \in \Lambda \mid t \realUP{\termenv} \varphi \}\qquad
%    \realF{\varphi} \defeq \{ t \in \Lambda \mid t \realU \varphi\}
%  \]
%  it is an alternative writing for $\cod{\varphi}$.
%  \begin{align*}
...
\begin{notation}
  ...
  Thus, when realizing a formula like $\forall \varx^{\{S\}}, \varphi$, it suffices to suppose that there is an objet $s \in \bS$ with a code $t$ and to prove $\varphi$ using the data $t$.
