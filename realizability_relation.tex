...
The only thing left to be defined is the $\cod -$ operation. We define it in the case where $\sem{\Prop}\defeq \SAT$, but if $\sem{\Prop}\defeq \SATE$, the same definition applies.

\begin{definition}[Code of an element]
  Let $\sigma \in \Oracle$. We define by induction, for any $S \in \Sort$ and any element $s \in \bS$, the set $\codE s \in \SATE$:
  \begin{itemize}
  ...
  \item if $S = \Prop$, then for all $\varA_{\enviro \in \Oracle} \in SAT$,
    \[\codE{(A)_{\enviro}} \defeq A_\enviro\]
  \end{itemize}

  We define then $\cod s$ by $\cod s \defeq \enviro \mapsto \codE s$.
\end{definition}
...
The following propositions justifie that the definitions give saturated sets, in the sense that they are monotonous with regard to the ordre on reduction contexts.
...
\begin{proof}
  ...
  \item if $S = T \to U$, then suppose that $t \realPP \Nat(\termt)$. To prove the expected result, let $\enviroTT \supOr\enviroT$ and $u \in \codP{\termenv(\termt)} \enviroTT$, then $tu \in \codP{\termenv(\termt)}{\enviroTT}$ by hypothesis on $t$.
