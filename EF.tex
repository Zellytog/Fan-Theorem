...
\begin{proposition}\label{prop:EF_notre}
  ...
  Then the triple $(\Phi,E,\relEFdot)$ is an evidenced framr.
\end{proposition}

\begin{proof}
Observe that $\varA \infSAT \varB$ is exactly the proposition $\relEF t {\varA}{\varB}$
for which we erase the witness $t$.
 The different components are thus defined by using the structure of $\SAT$ (but taking care of evidences which provided
...
$t \realU \varphi$ if and only if $\relEF{t}{\top}{\realUF{\varphi}}$.\emnote{il faut pas $\lambda\_.t$ ?}
