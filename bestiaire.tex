...
For a decidable formula $\varphi(\varx^S)$, we write $\eval\;\varphi$ for a term deciding $\varphi$. This is a term with witnesses that for $s \in \bS$ and $t \in \cod s$, $\eval\;\varphi \;t$ realizes either $\varphi(s)$ if it reduces to $\rtt$, or $\lnot\varphi(s)$ if it reduces to $\rff$.
...
By the definition of realizing $\exists \varn^{\{\Nat\}}$, for any $\alpha \real \Nat \to \Nat$, we can retrieve $\pi_1(b(\alpha))\real \Nat(\varn)$ such that $\pi_2(b(\alpha))\real B(\restr \alpha \varn)$. To lighten the notations, we write $\restr\alpha b$ for $\restr{\alpha}{\pi_1(b(\alpha))}$, which is thus a realizer of some list $\ell \in \List(\Bool)$ coming with a realizer of $B(\restr \alpha b)$.
...
Our goal is to construct a monotonous subset $C\subseteq B$ which is decidable. For this, it suffices to find a bounded formula such that any $\ell$ satisfying this formula is in $B$. From the previous remark, we only need to show that if $C(\ell)$, then there is a prefix of $\ell$ of the form $\restr\alpha b$.
...
$n_{C_b}\;n \redRT n$ if $C'_b(n)$ and
$n_{C_b}\;n \redRT n_{C_b}\;(n+1)$ otherwize,
