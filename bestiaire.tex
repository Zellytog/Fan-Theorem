...
Remark that if $\alpha_b\ListInf\ell$, then by the realizer of the monotonicity of $B$ and the fact that $B(\restr \alpha b)$, we can deduce (in the realizability sense) that $B(\ell)$ holds.
...
\[C_b(\ell) \defeq \eval (\exists \ell'^{\List(\Bool)}, |\ell'| < |\ell| \land \restr{\ell'0^\infty} b \ListInf \ell)\]
