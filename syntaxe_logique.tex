...
\begin{definition}[HOL-context]
  We fix a denumerable set $\Xt$ of variables which we denote by $\varx,\bvary,\ldots$ A (HOL-)context is a list of pairs $(\varx : S)$ where $\varx \in \Xt$ and $S \in \Sort$. We denote the set of (HOL-)contexts by $\Hctx$ and will write its elements $\Gamma,\Delta,\ldots$
\end{definition}
