...
\begin{definition}[Semantic]
  ...
  $
  where we interpret each term by its natural meaning. For the case of $\Gamma\vdash \sortPred{\termt} : \Prop$, the semantic is given by $\cod - \circ \sem{\Gamma\vdash \termt : S}$.
  %by the following interpretation:\ciseau
  %\begin{itemize}
...
%     \end{multline*}
  ...
  %\end{itemize}
  %where $\termrec_{\bN}$ (resp. $\termrec_{\bB}$, resp. $\termrec_{\bL}$) are the canonical recursors given by the initial algebra structure of $\bN$ (resp. $\bB$, resp. $\sem S^\star$), $\smallfrown$ is the operation which, given a word $u$ and a letter $a$, appends $a$ to $u$ to make the new word $ua$, $\implH$ is the implication of $H$ and $\landH$ is its meet.
