...
\begin{definition}[Semantic]
  Let $(H,\leqH)$ be a Heyting algebra with an intersection and a union operations.
  ...
  \end{itemize}
  where $\rec_{\bN}$ (resp. $\rec_{\bB}$, resp. $\rec_{\bL}$) are the canonical recurcors given by the initial algebra structure of $\bN$ (resp. $\bB$, resp. $\sem S^\star$), $\smallfrown$ is the operation which, given a letter $a$ and a word $u$, appends $a$ to $u$ to make a new word, $\implH$ is the implication of $H$, $\landH$ is its meet, $\bigcap$ is its intersection and $\bigcup$ its union.
