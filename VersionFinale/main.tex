...
\documentclass[a4paper,UKenglish,cleveref,enumerate, autoref, thm-restate]{lipics-v2021}
...
\FXRegisterAuthor{cc}{acc}{\color{red}}
...
\begin{proof}
  ...
  \emnote{Il manquait ceci:} Assuming that $n_{C_b}\;0$ indeed reduces to an integer $n_0$, let us describe how to implement a term $t_0$ such that $t_0 \realP{\enviro}{} \forall \alpha^{\{\Nat \to \Bool\}}, B(\restr \alpha {n_0})$. Let $\alpha:\Nat\to\Bool$ be any function realized by a code $e_\alpha$. By definition of $n_{C_b}$, in particular $C(\restr \alpha {n_0})$ holds. This means that during the enumeration, the algorithm found a list $\ell'$ such that $|\ell'|<n_0$ and $\restr {\ell'0^\infty} b \ListInf \restr \alpha {n_0}$.
...
\begin{proof}
  ...
  By hypothesis, the evidenced frame has unbounded search, so we can apply it to $\lnot \circ C'_b$ (because we search for a $n$ such that $C'_b(n) = 1$). Combining it with the code $e_{C'_b}$, this gives an evidence of a uniform bound $n_0$. \emnote{ajout} By a similar reasoning, we can mimic the definition of $t_0$ (in the proof of \Cref{thm:FT}).
