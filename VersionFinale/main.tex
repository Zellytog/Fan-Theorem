...
\begin{theorem}[Realization of Fan Theorem]\label{thm:FT}
  We have $\lambda b. \langle n_{C_b}\;0 , \lambda \alpha.\pi_2 \,(b\, \alpha)\rangle \realU \FT$, using the previous definition of $n_{C_b}$ .\emnote{la deuxième composante est fausse}
\end{theorem}

\begin{proof}
  Let us prove that $\lambda b. \langle n_{C_b}\;0 , \lambda \alpha.\pi_2 \,(b\, \alpha)\rangle \realU\FT$. Let $\enviro\in\Oracle$ be a reduction context of length $m$ and
  ...
  $\langle n_{C_b}\;0 , \lambda \alpha.\pi_2 \,(b\, \alpha)\rangle \realP{\enviro}{} \exists n^{\{\Nat\}}, \forall \alpha^{\{\Nat \to \Bool\}}, B(\restr\alpha n)$
  ...
  %In that case, since $B$ is closed by extension, $ \lambda \alpha.\pi_2 \,(b\, \alpha)$ thus realizes
  \emnote{en réalité, pour la second composante, il faudrait garder la trace de la liste qui justifie que un élément $\restr \alpha {n_{C_b}\,0}$ est dans $C$}
...
\begin{proof}
  ...
  By hypothesis, the evidenced frame has unbounded search, so we can apply it to $\lnot \circ C'_b$ (because we search for a $n$ such that $C'_b(n) = 1$). Combining it with the code $e_{C'_b}$, this gives an evidence of a uniform bound. Thus, all that is left to prove is that $\lnot \circ C'_b$ indeed has a $0$.
