...
since for instance it is unable to distinguish between
a statement and its contrapositive (e.g. dependent choice and
the bar induction principles).
The case of (Weak) Kőnig's Lemma (WKL) and Fan Theorem (FT)
is particularly interesting in that regard:
while WKL is well-known to imply FT, and if constructivists like
...
bound to classify theorems that are compatible with classical logic (which is not the case for all intuitionistic theorems), and is unable to distinguish between a statement and its contrapositive.
...
The Fan Theorem states that if such a set is barred, then it is uniformly barred. This can be understood as the contrapositive of Weak \Konig's Lemma (\WKL), which states that if a tree is infinite then it contains an infinite path.
...
In this work, we provide a computational interpretation of the Fan Theorem in line with our manifesto. To this end, we construct a realizability model that validates \FT while refuting \WKL, and we subsequently show that the induced interpretation is itself robust.
...
The HOL term constructors are akin to system T: there are constructors for abstraction and application for functions, constructors for pairing and projections, canonical constructors and recursor the different data types ($\Nat,\Bool,\List$). The main difference lies in the logical component, as we also add propositional constructors for the formulas above.
...
\begin{definition}[Semantics]\label{def:sem}
  ...
  For each sort $S \in \Sort$, we associate a set $\sem S$ (which we will also write $\bS$):
  \begin{multicols}{2}
  \begin{itemize}
  ...
  \end{itemize}
  \end{multicols}
  \noindent Suppose given a function associating to each object a set of codes (possibly empty):
  \[\begin{array}{ccccc}
  ...
  \item $\sem{\Gamma\vdash \termpi_2(\termt) : T} \defeq \pi_2 \circ \sem{\Gamma\vdash \termt : S \times T}$
...
using a semantics similar to Kripke semantics, the trees are forced to be
compatible with paths yet to be defined.
Then, we add infinite paths in the computational model by fixing
primitives $\oracle$ together with a reduction rule $\oracle\;\encode n \redi \encode{\alpha(n)}$,
...
and the Kripke semantics allows $b$ to take as argument the term $\oracle$.
Thus, we wish our Kripke-like semantics to be defined such that $b$ realizing a bar on $B$ not only means that $b$ computes as a bar for some given oracles, but also that it does so when new oracles are added. Finally, using a continuity argument, computing $b(\oracle)$ can be done only for a finite part of $\oracle$, which was already treated in the computation of the original algorithm, giving that both $B(\oracle)$ holds (because it extends the prefix given by a bar on $B$) and that $B(\oracle)$ does not hold (by our choice of $\alpha$). This rules out this case, which shows that the algorithm necessarily terminates.
\end{itemize}
...
In Lubarsky and Rathjen's model\cite{LubRat13}, the argument mentioned above is implemented by taking a Kripke model of IZF. In this model, a realizability model is constructed based on Turing machines with oracle. The Kripke model itself uses as frame the set $\omega^{<\omega}$, and each world $\enviro$ corresponds to a finite set of oracles. Through a well-designed construction, Lubarsky and Rathjen manage to construct the model so that the realizability model taken at some world $\enviro$ are Turing machine computing with oracles in $\enviro$.

In comparison, our model relies on the $\lambda$-calculus, mainly to allow more explicit constructions (\textit{e.g.} real concrete terms can be given and proved to realize some given sentences).
...
\begin{definition}[Lambda-terms]
  ...
  \begin{align*}
    ...
    & \mid \nil \mid t\append u \mid \rec_{\List}\;t\;u\;v
     \mid \oracle_n
  \end{align*}
  where $n$ ranges over $\bN$. We assume the usual convention of $\alpha$-renaming of bound variables.
\end{definition}



Using our multiple worlds analogy, we parametrize the reduction on terms by a list of functions $\bN \to \bB$. In place of the frame $\omega^{<\omega}$ previously mentioned, our set of worlds is $(\bN \to \bB)^{<\omega}$, but every definition could be restricted to a subset of $\bN \to \bB$, for example by taking only functions of some given degree. The model of \cite{LubRat13}
...
where $\encode{n}$ means $\lamS^n\;\lamZ$, the canonical encoding of integers in this $\lambda$-calculus, and the encoding of booleans is $\rtt$ and $\rff$, corresponding respectively to $\top \in \bB$ and $\bot \in \bB$.
...
\begin{definition}[Reduction]
  ...
  The reduction relation $\redE$ is the smallest relation containing $\rediE$ and which is compatible, meaning that it is closed by subterms. We write $\redERT$ for the reflexive and transitive closure of the relation $\redE$.
\end{definition}
...
\begin{proof}
  It suffices to show that $\rediE\subseteq \redi_{\enviroT}$, which is straightforward by comparing the list of redexes of both reductions.
\end{proof}
...
The notion of truth value in our model is thus, in a world $\enviro$, the set of saturated sets for the relation $\rediE$. This truth value is local, in the sense that it only speaks about $\enviro$, so the adapted notion of truth value (for the Kripke interpretation) is given by family of saturated sets, for each $\enviro$.
...
The fact that the operations are well defined is standard and uses the compatibility of the relation.
  ...
  \item Suppose there exists $t$ such that for any $\enviro\in\Oracle$ and $u \in \termAE \landSAT \termBE$, $tu \in \termCE$. Then,\linebreak let \mbox{$\enviro,\enviroT\in\Oracle$} be such that $\enviro\infOr \enviroT$ and $u \in \termAE$. By inclusion, $u \in \termA{\enviroT}$, so for any $v \in \termB{\enviroT}$, we have $t\;\langle u,v\rangle \in \termC{\enviroT}$. By antireduction, we therefore conclude that $\lambda x\;y.t\;\langle x,y\rangle$ is a witness that \mbox{$\varA\infSAT \varB \impliesSAT \varC$}.
  \end{itemize}
  Hence $\SATE$ and $\SAT$ are Heyting pre-algebras.
\end{proof}

\begin{remark}\label{rmk:inclusion}
  The preorder $\infSATE$ contains $\subseteq$ with the witness $t = \lambda x.x$. For this reason, the operator $\bigcap$ defines a lower bound, even though not the greatest.
\end{remark}
...
The realizability relation relates the $\lambda$-calculus and the HOL model. The first step to relate the two is to instantiate the semantic defined previously with a Heyting prealgebra interpreting $\Prop$. As is expected, this instantiation is done by taking $\sem{\Prop}$ to be the set of saturated sets. Be it $\SATE$ or $\SAT$, those sets are equipped with a Heyting prealgebra structure and both intersection and union operations.
...
The fact that $\codE s \in\SATE$ is a straightforward induction on the set of sorts. We postpone the proof that $\enviro\infOr \enviroT \implies \codE s \subseteq \codP s {\enviroT}$ to the  \Cref{prop.monotonicity}, as it is a particular case of a proposition of the realizability interpretation.
...
\begin{notation}
  We define the relativized universal quantification of a proposition $\varphi$ \linebreak
  as
  $\displaystyle\forall \varx^{\{S\}}, \varphi \defeq \forall \varx^S, S(\varx) \to \varphi$.
  Thus, when realizing a formula like $\forall \varx^{\{S\}}, \varphi$, it suffices to suppose that there is an object $s \in \bS$ with a code $t$ and to realize $\varphi$ using the data $t$.
...
The proof of the adequacy lemma proceed by induction on the typing relation. For the cases involving universal (resp. existential) quantification, an additional lemma is required to relate the realizability of $\varphi[\termt/\varx]$ and the realizability of $\varphi$ where $\rho$ is enriched by $\varx\mapsto \rho(\termt)$.
...
\begin{proof}
 ...
 \begin{itemize}
 \item if $\varphi = \sortPred{\termu}$, the equality on terms entails  $t \in \codE{\termenv(\termu[\termt/\varx])} \iff t \in \codE{\termenv[\varx\mapsto \termenv(\termt)](\termu)}$.
...
\begin{figure*}[t]
  ...
  \begin{mathpar}
   ...
   \scalebox{0.88}{\ruleAxL{$\!\Gamma\mid\Xi \vdash \rec_{\List}\! :\! \forall X^{\List(S) \to \Prop}\!, \sortPredS{X}{\termnil} \to (\forall \varx^{\{S\}}\!, \forall \vary^{\{\List(S)\}}\!, \sortPredS{X}{\vary} \to \sortPredS{X}{\varx \termcons \vary}) \to \forall \varx^{\{\List(S)\}}\!\!, \sortPredS{X}{\varx}$}{$\List_\mathrm e$}}
...
\begin{proof}
 ...
 \newcommand{\Cases}[1]{\emph{Cases \emph{(#1)}.}}
  ...
  Using the induction hypothesis, for any $\termenv' \models \Gamma, \varx : S$,  we get that $t[t_i/x_i] \realP{\enviro}{\termenv'} \psi$. For any $s \in \bS$, $\termenv[\varx \mapsto s] \models \Gamma$, so $t[t_i/x_i] \realP{\enviro}{\termenv[\varx\mapsto s]} \psi$ which is, by definition, $t[t_i/x_i] \realPP \forall \varx^S,\psi$.
  ...
  By induction hypothesis, we know that $\sem{\termt}(\termenv) \in \bS$ and that $t \realP{\enviro}{\termenv[\varx \mapsto \sem{\termt}(\termenv)]} \varphi$, so we indeed have some $s \in \bS$ (namely $\sem{\termt}(\termenv)$) such that $t \realP{\enviro}{\termenv[\varx\mapsto s]}\varphi$, which is the expected result.
  \item \case{$\exists_\mathrm e$}{$u\;t : \psi$} By induction hypothesis, we know that for any $\termenv\models \Gamma, \varx : S$, we have $u \realPP \varphi \to \psi$. In particular, by induction hypothesis there exists $s \in \bS$ such that $t \realP{\enviro}{\termenv[\varx \mapsto s]} \varphi$, but then $\termenv[\varx\mapsto s]\models \Gamma, \varx : S$ so we can apply $t$ to $u$ to find that $u\;t\realPP \psi$.
  ...
  \item \Case{$\List_\mathrm e$} Let $X$ be a predicate on $\List(S)$, $\enviroT \supOr\enviro$ and
    ...
    \begin{itemize}
    \item if $\sem{\varx}(\termenv) = \nil$, then $w \redRTP{\enviroT} \nil$, $\rec_{\List}\;u\;v\;w \redRTP{\enviroT} u$ and $u \realP{\enviroT}{\termenv} X(\termnil)$. In particular, we get that $\rec_{\List}\;u\;v\;w\realP{\enviroT}{\termenv} X(\termnil)$.
...
the false proposition $\bot \defeq \forall \varphi^{\Prop}, \varphi$
...
We assume given the function $\alpha \mapsto \restr \alpha \varn$, where $\restr \alpha \varn$ is the
...
The way trees are formalized can vary, but in reverse mathematics and computability
...
\WKL&\defeq\forall T^{\Tree^\Bool}\!\!\!, (\forall \varn^{\{\Nat\}}, \exists \ell^{\{\List(\Bool)\}}, |\ell| = \varn \land T(\ell)) \Rightarrow
...
Using boolean operations, it is straightforward that formulas of propositional logic over the atomic formulas
$t = u $ and $t \leq u$
...
Given a function $\alpha : \Nat \to \Bool$ and a number $n$, we write $\restr \alpha n : \List(\Bool)$ for the list of the $n$ first values of $\alpha$. Conversely, given a list $\ell : \List(\Bool)$, we write $\ell0^\infty$ for the function $\Nat \to \Bool$ which associates $\ell_i$, the $i$-th value of $\ell$, to $i < |\ell|$, and $0$ to any $i \geq |\ell|$.
...
We will now define the term realizing $\FT$.
...
We then use this with a bounded quantification: we enumerate every list $\ell$ and, each time, add to $C$ the list $\restr{\ell0^\infty} b$ and every extension, as long as those extensions are of size lower than $\ell$. This can also be rewritten as follows: given a list $\ell$, checking whether $\ell \in C$ amounts to checking whether there exists a list $\ell'$ of size lower than $|\ell|$ such that $\restr{\ell'0^\infty}{b}\ListInf \ell$.

This automatically gives a term
\[C_b(\ell) ~~\defeq~~ \eval_{\exists \ell'^{\{\List(\Bool)\}}, |\ell'| < |\ell| \land \restr{\ell'0^\infty} b \ListInf \ell}\,\ell\]
Now, we define the following predicate
\[C'_b(n) ~~\defeq~~ \eval_{\forall \ell^{\{\List(\Bool)\}}, |\ell| = n \implies C(\ell)}\, n\]
...
\begin{remark}\label{rmk:termin_FT}
  By computing reduction, we see that $n_{C_b} n \redRT n$ if $C'_b(n)$ holds, and that otherwise $n_{C_b} \redRT n_{C_b}\;(n+1)$.
  Hence, either $n_{C_b}\;0$ runs infinitely many steps, and for every $n$, $C'_b(n)$ is false (meaning that there exists $\ell_n \notin C$ of size $n$) or $n_{C_b} 0 \real \Nat$ and it is a uniform bound.
\end{remark}
...
\begin{lemma}[Continuity]\label{lem:continuity}
  ...
  Then for any $\alpha \realP{\enviro}{} \Nat \to \Nat$, there exists $m\in \bN$ such that
  \[\forall (\beta \realP{\enviro}{} \Nat \to \Nat), (\forall i < m, \beta\;\overline i \eqredE \alpha\;\overline i) \implies t\;\beta \eqredE t\;\alpha\]
\end{lemma}
\begin{proof}
 ...
 By hypothesis, we know that $t\;\alpha \redERT \overline n$ for some $n \in \bN$. By confluence, it is possible to chose any reduction strategy which is normalizing. In particular, we can choose the leftmost reduction strategy:
  ...
  Thus, we are left to prove the result with $t[\alpha/x] \stratETP l \overline n$. By induction on the number of reduction steps, we prove that there exists modulus $m$ such that for all $\beta$ such which agrees on the prefix $\restr \alpha m$, then  $t[\beta/x] \stratETP l \overline n$. Let us proceed by case analysis on the first step reduction:
  \begin{itemize}
  ...
  \item the redex can involve $\alpha$. In this case, because of the reduction strategy, we find a right context $E[\;]$ and a term $u$ such that
    ...
    Now, by induction hypothesis, we find a prefix $\restr\alpha m$ such that for any $\beta$ with this prefix, $E[\overline{\alpha(i)}][\beta/x] \stratETP l \overline n$ and $u[\beta/x] \stratETP l \overline i$. If $i>m$, we can strengthen the condition by considering $i$  and the longer prefix $\restr \alpha i$. Then, for $\beta$ with this prefix:
...
\begin{theorem}[Realization of Fan Theorem]\label{thm:FT}
   The exists a term $\mathsf{fan}$ such that $\mathsf{fan}\realU \FT$.
% uniform realizer $t_{\mathsf{fan}}$ We have $\lambda b. \langle n_{C_b}\;0 , \lambda \alpha.\pi_2 \,(b\, \alpha)\rangle \realU \FT$, using the previous definition of $n_{C_b}$ .\emnote{la deuxième composante est fausse}
\end{theorem}

\begin{proof}
  Let us prove that there exists a term $t_0$ such that $\mathsf{fan} \defeq \lambda b. \langle n_{C_b}\;0 , t_0\rangle \realU\FT$. Let $\enviro\in\Oracle$ be a reduction context of length $m$ and
  ...
  $\langle n_{C_b}\;0 , t_0\rangle \realP{\enviro}{} \exists n^{\{\Nat\}}, \forall \alpha^{\{\Nat \to \Bool\}}, B(\restr\alpha n)$
  but, from \Cref{rmk:termin_FT}, we already know that $n_{C_b}\;0$ either reduces to an integer or diverges.  If it reduces to an integer, then this integer is a uniform bound for $C$, but as $C \subseteq B$, it follows that $n_{C_b}\;0$ is a uniform bound for $B$ as well.
  Assuming that $n_{C_b}\;0$ indeed reduces to an integer $n_0$, let us describe how to implement a term $t_0$ such that $t_0 \realP{\enviro}{} \forall \alpha^{\{\Nat \to \Bool\}}, B(\restr \alpha {n_0})$. Let $\alpha:\Nat\to\Bool$ be any function realized by a code $e_\alpha$. By definition of $n_{C_b}$, in particular $C(\restr \alpha {n_0})$ holds. This means that during the enumeration, the algorithm found a list $\ell'$ such that $|\ell'|<n_0$ and $\restr {\ell'0^\infty} b \ListInf \restr \alpha {n_0}$.
  In particular, $\pi_2\,(b\,({\ell'0^\infty})\realP{\enviro}{}B(\restr {\ell'0^\infty} b)$, and we finally obtain a realizer of $B(\restr \alpha {n_0} )$ using that $B$ is monotone. Using again a fixpoint and an enumeration of lists to find $\ell'$, this mechanism can indeed be implemented by a term $t_0$ which only depends on $b$.

  Now, suppose $n_{C_b}\;0$ diverges. This means that for any $n \in \bN$, there exists some word $w_n \in \bB^\star$ such that $\lnot C(w_n)$. By applying Weak König's Lemma (in our meta-theory), we find an infinite path
  $\alpha : \bN \to \bB$
  such that for all $n \in \bN, \lnot C(\restr \alpha n)$. We now enrich our reduction context with this alpha:
  ...
  \[\langle n_{C_b}\;0 , \lambda \alpha.\pi_2 \,(b\, \alpha)\rangle \realP{\enviro}{} \exists n^{\{\Nat\}}, \forall \alpha^{\{\Nat \to \Bool\}}, B(\restr\alpha n)\qedhere\]
...
In this section, we therefore seek for a general way to state the two conditions above
...
It is then easy to check that these definitions satisfy the necessary rules for the evidence relation $\relEFdot$.
\end{proof}
...
\item The functions $(\Nat \to \Nat) \to \Nat$ are continuous, in the sense that for any function $f : (\Nat \to \Nat) \to \Nat$ and $\alpha : \Nat \to \Nat$ with a code, there exists a modulus $n$ such that $\forall \beta, \restr\alpha n = \restr \beta n \implies f(\alpha) = f(\beta)$.
...
a preordered set $(W, \leq)$ and to give a functor $F : W \to \bC$.
This gives a guideline for our generalization: we construct a well-designed category for our work,
where morphisms preserve the information we expect, and consider a functor from a preorder to such a category.
...
of \Cref{thm:FT}, they are (very) strict morphisms which preserve every construction
...
\begin{multicols}{2}
  ...
  \end{itemize}
\end{multicols}
\noindent where the rules hold for all $\varphi,\psi,\chi\in\Phi_1$,$\vec\varphi\in\powerset(\Phi_1)$, $\witx,\wity\in E_1$, $n\in\bN$.
...
a preorder $\bC$ and constructing a functor $\bC \to \CatEFData$.
...
Another principle that is used in \Cref{thm:FT}, but not necessarily realized in an arbitrary EF, is the ability to conduct an unbounded search. To find the uniform bound, we use a fixpoint akin to the $\mu$ operator from \cite{Kleene1943}. This is a reasonable assumption: for instance, every PCA implements unbounded search thanks to the fixpoint combinator $Y$.
...
As for \Cref{thm:FT},
we implicitly assume a classical meta-theory satisfying \WKL,
which is in line with the standard approach in realizability to show the validity
...
\begin{proof}
  ...
  By hypothesis, the evidenced frame has unbounded search, so we can apply it to $\lnot \circ C'_b$ (because we search for a $n$ such that $C'_b(n) = 1$). Combining it with the code $e_{C'_b}$, this gives an evidence of a uniform bound $n_0$. By a similar reasoning, we can mimic the definition of $t_0$ (in the proof of \Cref{thm:FT}).
  Thus, all that is left to prove is that $\lnot \circ C'_b$ indeed has a $0$.
  ...
  \[g(b)\real \forall \alpha^{\{\Nat \to \Bool\}}, \exists n^{\{\Nat\}}, \restr \alpha n \in g(B)\]
...
\begin{remark}
  In fact, while the previous proof mimics the proof of \Cref{thm:FT}, it should even be possible to weaken the hypotheses. Indeed, in the proof, we only need that infinite computable trees with a code inside $\EFE$ have a future in which there is an infinite path (with a code).
\end{remark}
...
\begin{itemize}
\item The first example is the one given before. For each $\enviro \in \Oracle$, we can define the EFdata $\EFSATE$ from \Cref{rmk:EFSigma}. For $\enviro, \enviroT \in \Oracle$ such that $\enviro\infOr \enviroT$, we have a morphism of EFdata given by inclusion. For each $\enviro$, the EFdata $\EFSATE$ has continuity, strong existential and unbounded search, hence the fact that it satisfies $\FT$.

\item The second example is the historical one: the second Kleene algebra $K_2$ \cite{Kleene1965} satisfies the premises. The preorder is the trivial one, with only one element. Continuity is by definition of the PCA (it is the set of continuous functions $(\bN \to \bN) \to \bN$), unbounded search can be defined for any PCA by encoding the combinator $Y$ in combinatory logic. Any function $\bN \to \bN$ has a code, hence the condition that a path exists for any decidable tree encoded. Finally, strong existential is true for intuitionistic PCAs. We thus find back the usual result that $K_2$ satisfies $\FT$.

\item A third example is the model we based our generalization on: the model by Lubarsky and Rathjen \cite{LubRat13}.
\end{itemize}

Observe that the existence of a path in a possible future is essential. For example, taking the realizability model given by $\SATE$, with local computation at some context $\enviro\in\Oracle$, the model does not realize $\FT$. To see this, consider the Kleene tree relativized to $\enviro$-computable functions. For any $\enviro$-computable path, given by a code $e$, we can simulate $e$ for increasing time duration until the finite path gets out of the tree, which will always happen because the infinite path in the Kleene tree are all uncomputable. This thus gives a bar for the Kleene tree, but this bar cannot be uniform as the length of lists in this tree is unbounded.

Finally, we advocate that the restriction of working on EFdatas instead of just EF is not a limitation in practice.
...
To support our claim that this computational interpretation is indeed robust and does not rely on other peculiarities of the underlying $\lambda$-calculus,
...
principle $\varphi$ should follow the same path:
\begin{itemize}
...
such that a computational system satisfying $(a)$ always satisfies $\varphi$ in the
induced realizability model
\item among the computational systems satisfying $(a)$, we can find ones
that do not satisfy a strictly stronger principle than $\varphi$.
\end{itemize}

This methodology has been applied to $\FT$ in this article,
but can be transposed to any other logical principle:
the hierarchy of choice principles in \cite{BreHer21} is a natural
candidate, as it gives both a list of principles and a good criterion
...
Another interesting question arises when considering alternative versions of \FT. For example, it is possible to weaken the premise that \emph{any bar must be uniform} by only considering bars on $\Pi_0^1$ trees or on computable trees (such alternatives are considered in \cite{LubRat13}).
