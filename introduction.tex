...
In this paper, we focus on the lowest principles of Brede-Herbelin hierarchy, namely Brouwer's Fan Theorem (\FT). Considering trees as prefix-closed sets of lists of natural numbers, a set $C$ whose complement is a tree is said to be \emph{barred} if it contains a finite prefix $\alpha_n$ of any infinite sequence $\alpha\in\bN^\bN$. It is \emph{uniformly barred} if there exists a natural number $n$ such that for any $\alpha\in\bN^\bN$, $\alpha_n\in C$.
