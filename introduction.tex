...
Here, in order to better stress the computational behavior of realizers, in \Cref{s:realizability} we rather consider a $\lambda$-calculus extended with oracles and we use it to to define a realizability interpretation in the spirit of relative realizability~\cite{VanOosten08}. In \Cref{s:FT}, we prove that our system satisfies a continuity property and use it to exhibit a simple $\lambda$-term which realizes \FT. We also show that our model refutes \WKL, thus providing a separation result. Last, we identify in \Cref{s:robust} the computational features of our construction, namely a continuity property, strong existentials and unbounded search, and we provide a robust version of the validity of \FT using evidenced frames.
