...
\emph{``any computational system providing an implementation of \emph{(a)} will induce an evidenced frame validating \emph{(b)}}''. In particular, Cohen \emph{et al.} proved that memoization techniques (a) in any of a variety of computational systems would lead to a realizability model of Countable Choice (b)~\cite{CohFarTat19,CohMiqTat21s}, which is a much stronger result than the definition of a specific model of the same axiom in ~\cite{Herbelin12,Miquey18a} based on an ad-hoc variant of the $\lambda$-calculus with some implementation of memoization.
...
In this paper, we focus on the lowest principle of Brede-Herbelin hierarchy, namely Brouwer's Fan Theorem (\FT). Considering trees as prefix-closed sets of lists of booleans
%natural numbers,
a set $C$ whose complement is a
%finitely branching
tree is said to be \emph{barred} if it contains a finite prefix $\restr \alpha n$ of any infinite sequence %$\alpha\in\bN^\bN$.
...
In this work, we aim to provide a computational interpretation of the Fan Theorem while following our manifesto: we therefore provide a robust realizability interpretation satisfying \FT while refuting \WKL.
...
To highlight a robust computation interpretation of the Fan Theorem, we proceed as follows. First, in order to better stress the computational behavior of realizers, we introduce in \Cref{s:HOL} a computationally meaningful syntax for logical statements on trees,
and we consider in \Cref{s:realizability}  a $\lambda$-calculus extended with oracles. We use it to define a realizability interpretation in the spirit of relative realizability~\cite{VanOosten08}. In \Cref{s:FT}, we prove that our system satisfies a continuity property and use it to exhibit a simple $\lambda$-term which realizes \FT. We also show that our model refutes \WKL, thus providing a separation result.

In a second phase, we identify in \Cref{s:robust} the computational features of our construction, namely a continuity property for the realizers, strong existentials and unbounded search, and we provide a robust version of the validity of \FT using evidenced frames, emphasizing that any other computational system featuring the same ingredients would induce a valid interpretation of \FT.
