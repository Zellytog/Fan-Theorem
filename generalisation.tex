...
we also give\emnote{require?} an encoding (which is the one we expect for assemblies)
...
\emnote{to do}having a strong notion underlying the quantifier $\exists$, that we call explicit existential. The explicit existential allows one to use a proof of $\exists x, \varphi$ to extract an object $a$ such that $\varphi$ is true on $a$. In the presence of effects, and even more so with classical logic, this explicit existential is not realized \cite{Herbelin05}.
...
in constructive settings\tlnote{insérer citation thèse Baillon}.
