...
\begin{proof}
  ...
  \item the other three cases are straightforward by using the induction hypothesis to replace $\realPP \varphi[\termt/\varx]$ by $\realP{\enviro}{\termenv[\varx\mapsto\termenv(\termt)]} \varphi$.
  \end{itemize}
  Hence the result.
\end{proof}
...
\begin{notation}
  A lot of functions of our model are not described by any HOL-term but can have a realizer (as any computable function $\bN \to \bN$ is the code of a function, but not all functions correspond to terms). Thus, we introduce the following notation for $t \in \Lambda, S \in \Sort, s \in \bS$:
