...
\end{proposition}
\begin{proof}
 The main point is show that $\infSAT$ and $\landSAT$ are adjoints, see Appendix \ref{app:realizability}.
\end{proof}
r
...
\end{proposition}
\begin{proof}See Appendix \ref{app:realizability}.
\end{proof}
\begin{notation}
...
\begin{notation}
  ...
  \twoelt{\pi_1 \realU \varphi \land \psi \to \varphi }{ \pi_2 \realU \varphi \land \psi \to \psi}
...
\begin{remark}\label{rmk:termin_FT}
  ...
  so either $n_{C_b}\;0$ runs infinitely many steps, and for every $n$, $C'_b(n)$ is false (meaning that there exists $\ell_n \notin C$ of size $n$) or $n_{C_b} \real \Nat$ and it is a uniform bound.
\end{remark}
...
\begin{remark}\label{rmk:EFSigma}
  ...
  with the same functions and evidenced.
\end{remark}
...
\begin{itemize}
\item a set $X$\hspace{2cm}
$\bullet$ a function $E_X : X \to \Phi$
\end{itemize}
