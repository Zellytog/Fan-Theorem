...
With this kind of statement, we expect to capture the exact computational content of $\FT$. More generaly, we expect that an exploration of logical principles in intuitionnistic reverse mathematics can be conducted by this methodology, that we can summarize as such to find the computational content of a logical principle $(b)$:
\begin{itemize}
\item a general definition of computational system must be given, and this computational system can be made into an EF (or another general realizability framework, like implicative algebra \cite{Miquel20})
\item on this notion of computational system, we can define a condition $(a)$ such that a computational system satisfying $(a)$ always satisfies $(b)$ in its realizability model
\item among the computational systems satisfying $(a)$, we can find ones that do not satisfy a strictly stronger principle than $(b)$.
\end{itemize}
This methodoly has been applied to $\FT$ in this methodoly, but can be transposed to any other logical principle: first, the hierarchy of choice principles in \cite{BreHer21} is a natural candidate, as it gives both a list of principles and a a good criterium to exhibit what strictly stronger principle we do not expect to satisfy (namely the principles above in the hierarchy, and the classical counterpart of intuitionnistic principles).
...
\begin{proof}\textsc{Of \Cref{prop.monotonicity}.}
