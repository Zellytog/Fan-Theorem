...
\begin{notation}
  ...
  $\sem{\termt}(\rho)$ for the canonical element in $\bS$ associatied to this term.
\end{notation}
...
\begin{center}
  \begin{tabular}{l|l}
    ...
    &
    \multirow{2}{8cm}{$t \realPP \forall \varx^S, \varphi \defeq \forall s \in \bS, t\realP{\enviro}{\termenv[\varx \mapsto s]} \varphi$
      \\
...
\end{center}
  ...
  We also define the universal realizability relation by
...
\begin{notation}
  ...
  The relativised existantial quantification is defined by $\exists \varx^{\{S\}}, \varphi \defeq \exists \varx^{S}, S(\varx) \land \varphi$.
...
%  The proof of adequacy is done by an induction on the typing relation $\Gamma\mid \Xi\vdash t : \varphi$ and is standard in realizability methods, see Appendix \ref{app:realizability} for a complete proof.
 The proof is by induction on the typing derivation:%\todo{"case" en italique, etc..}
  \begin{itemize}
  \item \textit{case Ax with $t = x_i$:} the result is by hypothesis.
  \item \textit{case $\to_\mathrm i$ with $\lambda x.t : \psi \to \chi$:} let $\enviroT \supOr\enviro$ and $u \realP{\enviroT}{\termenv} \psi$, let us prove that $(\lambda x.t)u \realP{\enviroT}{\termenv} \chi$. By hypothesis, $t[t_i/x_i][u/x] \realP{\enviroT}{\termenv} \chi$, but then it suffices to use the fact that $\realFP{\chi}{\termenv}{\enviroT}$ is closed by antireduction.
  \item \textit{case $\to_\mathrm e$} is straightforward by definition of realizing $\psi \to \chi$.
  \item \textit{case $\land_\mathrm i$ with $\langle t,u \rangle : \psi \land \chi$:} we know by induction hypothesis that $t[t_i/x_i] \realPP \psi$ and that $u[t_i/x_i] \realPP \chi$. Moreover, $\pi_1\; \langle t,u \rangle \redE t$ and $\pi_2\;\langle t,u \rangle \redE u$, so $\pi_1\;(\langle t,u\rangle[t_i/x_i]) \realPP\psi$ and $\pi_2\;(\langle t,u\rangle[t_i/x_i]) \realPP \chi$ by saturation, hence $\langle t,u\rangle [t_i/x_i] \realPP \psi\land \chi$.
  \item \textit{both cases $\land_\mathrm e$} are straightforward by definition of realizing a conjunction.
  \item \textit{case $\forall_\mathrm i$ with $t : \forall \varx^S, \psi$:} by induction hypothesis, we know that for any $\termenv' \models \Gamma, \varx : S$, $t[t_i/x_i] \realP{\enviro}{\termenv'} \psi$. For any $s \in \bS, \termenv[\varx \mapsto s] \models \Gamma$, so $t[t_i/x_i] \realP{\enviro}{\termenv[\varx\mapsto s]} \psi$ which is, by definition, $t[t_i/x_i] \realPP \forall \varx^S,\psi$.
  \item \textit{case $\forall_\mathrm e$ with $t : \psi[\termt/\varx]$:} by definition of realizing $\forall \varx^S, \psi$ and using the substitution lemma.
  \item \textit{case $\exists_\mathrm i$ with $t : \exists \varx^S, \varphi$:} by induction hypothesis, we know that $\sem{\termt}(\termenv) \in \bS$ and that $t \realP{\enviro}{\termenv[\varx \mapsto \sem{\termt}(\termenv)]} \varphi$, so we indeed find some $s \in \bS$ (namely $\sem{\termt}(\termenv)$) such that $t \realP{\enviro}{\termenv[\varx\mapsto s]}\varphi$, which is the expected result.
  \item \textit{case $\exists_\mathrm e$ with $u\;t : \psi$:} by induction hypothesis, we know that for any $\termenv\models \Gamma, \varx : S$, $u \realPP \varphi \to \psi$. In particular, there is by induction hypothesis some $s \in \bS$ such that $t \realP{\enviro}{\termenv[\varx \mapsto s]} \varphi$, but then $\termenv[\varx\mapsto s]\models \Gamma, \varx : S$ so we can apply $t$ to $u$ to find that $u\;t\realPP \psi$.
  \item we won't treat the \textit{cases $\Nat$ nor $\Bool$}, as they are the same as the \textit{case $\List$}.
  \item \textit{case $\List_\mathrm i^{\nil}$}: indeed, $\nil \redERT \nil$.
  \item \textit{case $\List_\mathrm i^{\append}$}: by induction hypothesis.
  \item \textit{case $\List_\mathrm e$}: let $X$ be a predicate on $\List(S)$, $\enviroT \supOr\enviro$ and
    \[%\begin{cases}
...
\begin{notation}
  ...
  These notations can also be used with a reduction context, writing \textit{e.g.} $t\realP{\enviro}{} S$ for $\exists s \in \bS, t \realP{\enviro}{} S(s)$.
\end{notation}
...
which we call a monotone predicate:
...
For a decidable formula $\varphi(\varx^S)$, we write $\eval\varphi$ for a term deciding $\varphi$. This is a term with witnesses that for $s \in \bS$ and $t \in \cod s$, $\eval\varphi t$ realizes either
\begin{center}
  \begin{tabular}{l|l}
    $\varphi(s)$ if it reduces to $\rtt$
    &
    $\lnot\varphi(s)$ if it reduces to $\rff$.
  \end{tabular}
\end{center}
%\todo{2 colonnes}
...
This section ends with the statement of Theorem \ref{thm:main}, which is a more abstract
variant of Theorem \ref{thm:FT}: in this version, the realizability structure realizing $\FT$ can have realizers of any form ($\lambda$-terms, Turing machines\ldots) as long as it gives an implementation of integers with its recursor, satisfies the two conditions mentioned above for the argument to work,
...
\todo{remplacer par la figure papier MCA}
...
\begin{definition}[Evidenced Frame with data types]
  ...
  is a tuple $(\Phi,E,\relEFdot, \cod \cdot, e_0, e_S, e_{\rec})$ where:\todo{2 colonnes ?}
  \begin{itemize}
...
%    \[\cod f \defeq \prod_{x \in T} \PropImpl{\cod x}{\cod{f(x)}}\]
  \item $\relEF{e_0}{\top}{\cod 0}$ and for all $n \in \bN$, $\relEF{e_S}{\cod n}{\cod{n + 1}}$
...
\label{s:application}
Having Theorem \ref{thm:main} at hands, we give examples of realizability models satisfying the conditions (and thus, realizing $\FT$).
...
Is there a variation of the Theorem \ref{thm:main} accounting for such weakenings?
