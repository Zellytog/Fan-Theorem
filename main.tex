...
\todo{2 colonnes}
  ...
  \begin{itemize}
  \item $t \realPP \sortPred{\termt} \defeq t \in \codE{\sem{\termt}(\termenv)}$
...
\begin{proof}\todo{couper si besoin}
...
%  The proof of adequacy is done by an induction on the typing relation $\Gamma\mid \Xi\vdash t : \varphi$ and is standard in realizability methods, see Appendix \ref{app:realizability} for a complete proof.
 The proof is by induction on the typing derivation:\todo{"case" en italique, etc..}
  ...
  \item case $\to_\mathrm i$ with $\lambda x.t : \psi \to \chi$: let $\enviroT \supOr\enviro$ and $u \realP{\enviroT}{\termenv} \psi$, let us prove that $(\lambda x.t)u \realP{\enviroT}{\termenv} \chi$. By hypothesis, $t[t_i/x_i][u/x] \realP{\enviroT}{\termenv} \chi$, but then it suffices to use the fact that $\realFP{\chi}{\termenv}{\enviroT}$ is closed by antireduction.
  ...
  \item case $\exists_\mathrm i$ with $t : \exists \varx^S, \varphi$: by induction hypothesis, we know that $\sem{\termt}(\termenv) \in \bS$ and that $t \realP{\enviro}{\termenv[\varx \mapsto \sem{\termt}(\termenv)]} \varphi$, so we indeed find some $s \in \bS$ (namely $\sem{\termt}(\termenv)$) such that $t \realP{\enviro}{\termenv[\varx\mapsto s]}\varphi$, which is the expected result.
...
and $\ThRealU \defeq \{ \varphi \in \Prop \mid \exists t \in \Lambda, t \realU \varphi\}$
%}
...
\begin{itemize}
\item $\varphi(s)$ if it reduces to $\rtt$, or
\item $\lnot\varphi(s)$ if it reduces to $\rff$.
\end{itemize}
\todo{2 colonnes}
...
By the definition of realizing $\exists \varn^{\{\Nat\}}$, for any $\alpha \real \Nat \to \Bool$, we can retrieve $\pi_1(b(\alpha))\real \Nat(\varn)$ such that $\pi_2(b(\alpha))\real B(\restr \alpha \varn)$. To lighten the notations, we write $\restr\alpha b$ for $\restr{\alpha}{\pi_1(b(\alpha))}$, which is thus a realizer of some list $\ell \in \List(\Bool)$ coming with a realizer of $B(\restr \alpha b)$.
